        ...
        for(const gamepad of navigator.getGamepads()){
            if(gamepad){
                if(gamepad.axes.length >= 2){
                    const len = Math.sqrt(gamepad.axes[0] * gamepad.axes[0]  + gamepad.axes[1] * gamepad.axes[1]);
                    if(len > 0.16){
                ...
                if(gamepad.axes.length >= 4){
                    const len = Math.sqrt(gamepad.axes[2] * gamepad.axes[2]  + gamepad.axes[3] * gamepad.axes[3]);
                    if(len > 0.16){
                ...
                if(gamepad.buttons[4]?.pressed){
                    ...
                    if(cooldown < this.game.ticks){
                        const newSelection = (player.inventory.selection - 1) % player.inventory.items.length;
