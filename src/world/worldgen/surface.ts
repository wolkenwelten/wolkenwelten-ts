...
const grassHeight = (x: number, z: number):number => {
    ...
    let y = Math.min((900.0 - (d + dy + dmy)),((d + dy / 2.0) - (600.0 - 128.0 + duy))) / 16.0;
    if(y > 24.0) {
        y = 24.0 + (y - 24.0) * 23.45;
    }
...
const plantRock = (chunk: Chunk, x:number, gh:number, z:number, size: number) => {
            ...
            if(endY >= 0){
                if(gh < 1){
                    ...
                    if(endY < 32){
                        ...
                        if((gh > 4) && (gh < 23)){
                            if((x > 3) && (x < 29) && (z > 3) && (z < 29) && (rng.bool(155))){
                                const treeHeight = rng.int(8,11);
                                plantTree(chunk, x, gh, z, treeHeight);
                            }
                            if((x > 3) && (x < 29) && (z > 3) && (z < 29) && (rng.bool(392))){
