...
export class Character extends Entity {
    ...
    mayJump(world: World): boolean {
        return world.isSolid(this.x, this.y - 1.7, this.z);
    }

    isSolidPillar(world: World, x: number, y: number, z: number): boolean {
        return (
            world.isSolid(x, y, z) ||
            world.isSolid(x, y - 0.4, z) ||
            world.isSolid(x, y + 0.8, z)
        );
    }
    ...
    update(world: World) {
        ...
        if (this.movementY > 0 && this.mayJump(world)) {
            this.vy = 0.17;
        }

        if (this.isSolidPillar(world, this.x - 0.4, this.y - 0.8, this.z)) {
        ...
        if (world.isSolid(this.x, this.y - 1.7, this.z)) {
            this.vy = Math.max(this.vy, 0);
        }
        if (world.isSolid(this.x, this.y + 0.7, this.z)) {
            this.vy = Math.min(this.vy, 0);
        }

        if (this.isSolidPillar(world, this.x, this.y - 0.8, this.z - 0.4)) {
