...
const addFaceFront = (
    ...
    vertices.push(x + w, y + h, zd, texture_index, side | ((light >> 4) & 0xF0));
...
const addFaceTop = (
    ...
    vertices.push(x + w, yh, z + d, texture_index, side | ((light >> 4) & 0xF0));
...
const addFaceRight = (
    ...
    vertices.push(xw, y + h, z + d, texture_index, side | ((light >> 4) & 0xF0));
    ...
    for(let x=0;x<32;x++){
        for(let y=0;y<32;y++){
            for(let z=0;z<32;z++){
                ...
                if(b){
                    const bt = blocks[b];
                    if(!bt){throw new Error(`Unknown block type: ${b}`); }
